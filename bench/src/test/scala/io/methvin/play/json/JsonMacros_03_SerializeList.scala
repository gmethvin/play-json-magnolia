...
class JsonMacros_03_SerializeList {
  ...
  def setup(): Unit = {
    employees = (1 to 100) map { id =>
      Employee(id, s"Foo$id", s"Bar$id", "New York", "United States", Seq("a", "b", "c"))
    }
  }
  ...
  def toJsonMagnoliaMacro(): JsValue = {
    implicit val writes: Writes[Employee] = MagnoliaWrites.gen[Employee]
    employeesJson = Json.toJson(employees)
    employeesJson
  }
